...
Web App 会将最近 30 份完整走查报告保存到当前浏览器的 IndexedDB，包括页面截图、流程截图、DOM 快照和 Accessibility Tree。历史记录不会上传到服务器，也不会与其他浏览器、设备或访问者共享；用户可在“历史报告”页面清空本地记录。切换浏览器、清理站点数据或更换设备后，这些记录不会保留。

历史报告可再次查看截图，也可导出 Markdown、JSON、截图、DOM 快照和无障碍树。因为 DOM 快照可能包含页面中当时可见的表单内容，请只在受信任的浏览器和设备上运行走查。
