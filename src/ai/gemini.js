...
summary 必须像审计负责人写给团队的结论：总结问题模式、判断根因方向、指出应该从组件/设计变量/内容规范/交互状态哪一层修，而不是复述数量。
...
function buildPrompt(evidencePack) {
  return [
    ...
    '重点完成三件事：',
    '1. summary：输出有洞见的页面级审计结论。verdict 用 1-2 句话说明主要风险模式和产品影响；keyFindings 写 3-5 条“为什么会集中出现这些问题”的判断；recommendedNextSteps 写 3-5 条可执行修复策略。',
    '2. V1 语义复核：alt、链接文案、错误提示、图表说明、焦点顺序、状态反馈。',
    '3. V2 修复建议：为 evidencePack.issues 中的高优先级样本补充根因、用户影响、研发修复、UED 建议、示例代码、可复制 issue。',
    'summary 写作要求：',
    '- 不要复述总数、严重级别数量、规则层级数量。',
